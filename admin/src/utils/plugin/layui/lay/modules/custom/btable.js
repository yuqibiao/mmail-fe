                ...
                for (var i = 0; i < columns.length; i++) {
                    if (columns[i].format) {
                        $('#' + dataId).find('tr').each(function () {
                            ...
                            $field.html(columns[i].format(value, obj));
                        });
                    }
                }
                //执行回调函数
            ...
            success: function () { //完成的回调
                ...
                if (_config.checkbox) {
                    ...
                    $(_config.elem).find('tbody.btable-content').children('tr').each(function (e) {
                        var $that = $(this);
                        $that.children('td').eq(0).on('click', function () {
                            //只允许选择一行
                            if (_config.singleSelect) {
                                ...
                                var cbxTotal = $that.parent('tbody').children('tr').length;
                                /*console.log("===选中行数==cbxCount="+cbxCount);
                                 console.log("===总行数=cbxTotal=="+cbxTotal);*/
                                //---判断是否为全选
                                var allCheck = $(_config.elem).find('thead').children('tr').children('th:first-child').children('input[type=checkbox]')[0];

                                allCheck.checked = cbxCount === cbxTotal;
                            }
                            form.render('checkbox');
                        });
                    });
